...
require_once(ABSPATH.'/extensions/Authentication/adldap/adLDAP.php');

/**
* Active Directory authentication tools
*
...
* @copyright GPL
*/
class Authentication_adLDAP extends Authentication {
    /**
    ...
    */
    ...
    /**
    * Contains a domain name to base_dn pairing of
    * domains you wish to try authentication against
    *
    * @access private
    * @var array
    */
    ...
    /**
    * Creates an instance of Authenication_adLDAP class
    *
    ...
    */
    public function __construct() {
        ...
        $domains = array(
            "indianatech.net" => "DC=indianatech,DC=net",
            "indianatech.edu" => "DC=indianatech,DC=edu",
            "indtech.edu" => "DC=indtech,DC=edu",
        );

        $this->__set("domains", $domains);

        $this->ad->_account_suffix = '';
        $this->ad->_base_dn = '';
    }
    ...
    */
    public function login($username = 'guest', $password = '') {
        ...
        $sql = array (
            "mysql" => array (
                'info' => "SELECT * FROM `"._PREFIX."_users` WHERE `username`=':1' AND status='A' LIMIT 1",
        ...
        if (strpos($username, "@") !== FALSE) {
            $temp_username = explode("@", $username);
            $username = $temp_username[0];
        }
        ...
        $stmt = $this->db->prepare($sql[_DBSYSTEM]['info']);
        ...
        /**
        * We can have more than one domain and base DN so we check
        * each one and break out of the loop on the first one that
        * succeeds
        */
        foreach ($this->__get("domains") as $domain => $base_dn) {
            $this->ad->_account_suffix = '@' . $domain;
            $this->ad->_base_dn = $base_dn;
        ...
        if (!$success) {
            ...
            $_SESSION['home_dir']		= ($results['home_dir'] == "") ? _USER_ROOT . $results['username'] : $results['home_dir'];
            $_SESSION['group_dir']		= ($results['group_dir'] == "") ? _GROUP_ROOT . $results['username'] : $results['group_dir'];
