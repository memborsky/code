...
class most_active_visitors {
    ...
    */
    public function __construct() {
        ...
        */
        $this->__set("report_name", "most_active_visitors");

        /**
        * Set up description
        */
        $this->__set("report_desc", "Graphs most active visitors by IP address");
    }
    ...
    public function show() {
        ...
        switch($period) {
            ...
            case 'day':
                ...
                $report = $this->run($time_from, $time_till);
                ...
                $this->tpl->display('reports/most_active_visitors.tpl');
                break;
            case 'week':
                $time_till = time();
    ...
    * @access public
    */
    public function run($time_from, $time_till) {
        $sql = array (
            "mysql" => array(
                "ips" => "SELECT DISTINCT(ip) FROM "._PREFIX."_logging WHERE timestamp >= ':1' AND timestamp <= ':2' ORDER BY ip ASC",
                "count" => "SELECT count(log_id) FROM "._PREFIX."_logging WHERE ip = ':1' AND timestamp >= ':2' AND timestamp <= ':3'"
            )
        );

        $stmt1 = $this->db->prepare($sql[_DBSYSTEM]["ips"]);
        $stmt2 = $this->db->prepare($sql[_DBSYSTEM]["count"]);

        $stmt1->execute($time_from, $time_till);

        /**
        * Create the graph
        */
        $Graph =& Image_Graph::factory('graph', array(600, 500));
        $Graph->add(Image_Graph::factory('title', array('IP Address Hit Counter', 11)));
        ...
        while ($row = $stmt1->fetch_assoc()) {
            ...
            $stmt2->execute($ip, $time_from, $time_till);
        ...
        $AxisX->setTitle('IP Address');
        if ($stmt1->num_rows() >= 15)
            $AxisX->setFontAngle('vertical');

        $AxisY =& $Plotarea->getAxis(IMAGE_GRAPH_AXIS_Y);
        $AxisY->setTitle('Number of Hits', 'vertical');
