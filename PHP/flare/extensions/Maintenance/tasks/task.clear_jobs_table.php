...
class clear_jobs_table {
    ...
    */
    public function __construct($task_id = 0) {
        ...
        // Set the name of the report according to filename
        $this->__set("task_name", "clear_jobs_table");

        // Set up description
        $this->__set("task_desc", "Clears the jobs table in the Flare database");
    }
    ...
    */
    public function show() {
        ...
        $sql = array(
            "mysql" => array(
                "jobs" => "SELECT * FROM "._PREFIX."_jobs ORDER BY job_id ASC"
            )
        );

        $stmt1 = $this->db->prepare($sql[_DBSYSTEM]['jobs']);
        ...
        $this->tpl->display('tasks/task_clear_jobs_table.tpl');
    }
    ...
    */
    public function run() {
        $sql = array (
            "mysql" => array (
                "clear" => "DELETE FROM "._PREFIX."_jobs",
            )
        );
